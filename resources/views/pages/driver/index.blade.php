...
<div class="row">
    <div class="col-md-6 col-sm-12" id="breadcrumb-index">
        <h2 class="title-header">@lang('default.new.driver.index')</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb transparent">
                <li class="breadcrumb-item"><a href="{{route('driver.index')}}" style="color:#444444;">@lang('default.new.driver.index')</a></li>
            </ol>
        </nav>
    </div>
    <div class="col-md-6 col-sm-12" id="breadcrumb-create">
        <h2 class="title-header">@lang('default.new.driver.title-create')</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb transparent">
                <li class="breadcrumb-item"><a href="{{route('driver.index')}}" style="color: #444444;">@lang('default.new.driver.index')</a></li>
                <li class="breadcrumb-item active" aria-current="page">@lang('default.new.driver.sub-title-create')</li>
            </ol>
        </nav>
    </div>
    <div class="col-md-6 col-sm-12" id="breadcrumb-edit">
        <h2 class="title-header">@lang('default.new.driver.title-edit')</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb transparent">
                <li class="breadcrumb-item"><a href="{{route('driver.index')}}" style="color: #444444;">@lang('default.new.driver.index')</a></li>
                <li class="breadcrumb-item active" aria-current="page">@lang('default.new.driver.sub-title-edit')</li>
            </ol>
        </nav>
    </div>
...
@include('pages.driver.form')

@include('pages.driver.datatable')
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            saveData(e){
                ...
                formData.append('plate_number', self.data.plate_number)
                formData.append('vehicle_category_id', self.data.vehicle_category_id)
                formData.append('car_model', self.data.car_model)
                formData.append('image', self.data.image)
                formData.append('_token', "{{csrf_token()}}")


                self.postDataMultipart("{{route('driver.store')}}", formData)
            },
            updateData(e){
                e.preventDefault();
                let self = this,
                ...
                self.postDataMultipart("{{route('driver.index')}}/"+self.data.id+"", formData)
            },
            fetchData(url){
                let self = this
        ...
        mounted() {
            ...
            let dataTable = $("#datatable").DataTable({
                ajax: "{{route('driver.index')}}?type=datatable",
                processing: true,
                serverSide : true,
                responsive: true,
                ...
                buttons: [
                    ...
                    //         location.href = "{{route('driver.export')}}?type=csv"
                    //     }
                    // },
                    // {   extend: 'csv', text: "@lang('default.action.export.csv')",  bom: true,
                    //     title: 'Driver' + new Date().toISOString().slice(0, 10),
                    //     exportOptions: {
                    //         columns: [ 0, 3, 4, 5, 1],
                    //         format: {
                    ...
                    //     action: function ( e, dt, node, config ) {
                    //         location.href = "{{route('driver.export')}}?type=excel"
                    //     }
                    // },
                    {
                        text: "@lang('default.action.export.print')",
                        action: function ( e, dt, node, config ) {
                            location.href = "{{route('driver.export')}}?type=print"
                        }
                    }
